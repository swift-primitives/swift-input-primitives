...
extension Input {
    ...
    public protocol `Protocol`<Element>: Streaming, ~Copyable {
        ...
        ///
        /// Must be `Comparable` for range-based validation. Concurrent
        /// parsing relies on region-based isolation at the consumer boundary
        /// per [MEM-SEND-012]; protocol-layer associatedtypes do not pre-decide
        /// callers' isolation discipline with a `Sendable` constraint.
        ///
...
extension Input.`Protocol` where Self: ~Copyable {
    ...
    ///
    /// Default implementation uses ``bounds`` for validation.
    ///
